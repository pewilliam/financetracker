...
export default function InstallmentsPage({ categories = [], invoices = [], revision = 0, onNew, onDetails, onRequestDelete }) {
  ...
  useEffect(() => {
    ...
    listInstallmentPage({ tab: activeTab === "inProgress" ? "active" : "paid", search: debouncedQuery, categoryId: category === "all" ? "" : category, invoiceTemplateId: invoice === "all" ? "" : invoice, situation, sortBy, page, pageSize })
  ...
  }, [activeTab, debouncedQuery, category, invoice, situation, sortBy, page, pageSize, revision, retryToken]);
  ...
  const activeFilters = [query && { id: "query", label: `Busca: ${query}`, clear: () => { setQuery(""); setDebouncedQuery(""); setPage(1); } }, category !== "all" && { id: "category", label: `Categoria: ${categories.find((item) => String(item.id) === category)?.name}`, clear: () => updateFilter(setCategory)("all") }, invoice !== "all" && { id: "invoice", label: `Cartão/fatura: ${invoiceSources.find((item) => String(item.template_id ?? item.id) === invoice)?.name}`, clear: () => updateFilter(setInvoice)("all") }, situation !== "all" && { id: "situation", label: `Situação: ${{ regular: "Em dia", soon: "Vence em breve", overdue: "Atrasados" }[situation]}`, clear: () => updateFilter(setSituation)("all") }].filter(Boolean);
  const clearFilters = () => { setQuery(""); setDebouncedQuery(""); setCategory("all"); setInvoice("all"); setSituation("all"); setPage(1); };
  ...
  return (
    <section className="installments-page">
      ...
      {loadError && !data.items.length ? <div className="installment-empty-results primary-empty error"><span><AlertTriangle size={26} /></span><h3>Não foi possível carregar os parcelamentos</h3><p>Confira sua conexão e tente atualizar os dados.</p><button className="btn btn-primary" type="button" onClick={() => setRetryToken((value) => value + 1)}>Tentar novamente</button></div> : <>
        ...
        <div className="installment-tools"><label className="installment-search"><Search size={17} /><span className="sr-only">Buscar compra por nome</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nome..." />{query && <button type="button" onClick={() => { setQuery(""); setDebouncedQuery(""); }} aria-label="Limpar busca"><X size={15} /></button>}</label><button className={`btn installment-filter-trigger ${filtersOpen ? "active" : ""}`} type="button" onClick={() => setFiltersOpen((current) => !current)} aria-expanded={filtersOpen}><Filter size={16} /> Filtros {activeFilters.length > 0 && <span>{activeFilters.length}</span>}</button><div className={`installment-filter-fields ${filtersOpen ? "open" : ""}`}><label><span>Categoria</span><select value={category} onChange={(event) => updateFilter(setCategory)(event.target.value)}><option value="all">Todas as categorias</option>{categories.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label><span>Cartão ou fatura</span><select value={invoice} onChange={(event) => updateFilter(setInvoice)(event.target.value)}><option value="all">Todos</option>{invoiceSources.map((item) => <option value={item.template_id ?? item.id} key={item.template_id ?? item.id}>{item.name}</option>)}</select></label><label><span>Situação</span><select value={situation} onChange={(event) => updateFilter(setSituation)(event.target.value)}><option value="all">Todos</option><option value="regular">Em dia</option><option value="soon">Vence em breve</option><option value="overdue">Atrasados</option></select></label><label><span>Ordenar por</span><select value={sortBy} onChange={(event) => updateFilter(setSortBy)(event.target.value)}><option value="nextDue">Próximo vencimento</option><option value="remaining">Maior valor restante</option><option value="installment">Maior valor da parcela</option><option value="progress">Maior progresso</option><option value="newest">Mais recentes</option><option value="oldest">Mais antigos</option><option value="alphabetical">Ordem alfabética</option></select></label></div><div className="view-toggle installment-view-toggle" aria-label="Alternar visualização"><button type="button" className={viewMode === "cards" ? "active" : ""} aria-pressed={viewMode === "cards"} onClick={() => changeView("cards")}><Grid2X2 size={16} /> Cards</button><button type="button" className={viewMode === "table" ? "active" : ""} aria-pressed={viewMode === "table"} onClick={() => changeView("table")}><List size={16} /> Tabela</button></div></div>
