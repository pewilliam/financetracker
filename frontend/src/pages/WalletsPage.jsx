  ...
  return <div className="modal-layer wallet-modal-layer">
    ...
    <form className="modal-card wallet-modal wallet-editor-modal" onSubmit={submit}>
      ...
      <div className="wallet-modal-body form-stack">
        <div className="field-label"><span>Nome da carteira</span><input maxLength="100" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Ex: Conta principal" required autoFocus={shouldAutoFocusName} /></div>
        <div className="field-label"><span>Instituição <small>(opcional)</small></span><input maxLength="100" value={form.institution} onChange={(event) => setForm({ ...form, institution: event.target.value })} placeholder="Ex: Nubank" /></div>
        <div className="field-label"><span>Tipo</span><CategorySelect categories={WALLET_TYPE_OPTIONS} value={form.type} onChange={(type) => setForm({ ...form, type })} multiple={false} clearable={false} placeholder="Selecione o tipo" searchPlaceholder="Buscar tipo..." ariaLabel="Tipos de carteira" className="wallet-type-select" /></div>
        {!editing && <div className="field-label"><span>Saldo inicial</span><input inputMode="decimal" value={form.initial_balance} onChange={(event) => setForm({ ...form, initial_balance: formatTypedMoneyForEditing(event.target.value, language) })} onBlur={() => setForm({ ...form, initial_balance: formatTypedMoneyAsCurrency(form.initial_balance, language) })} placeholder={formatMoney(0, language)} /></div>}
        ...
        <div className="field-label"><span>Início do acompanhamento</span><DateField value={form.tracking_started_on} onChange={(value) => setForm({ ...form, tracking_started_on: value })} /></div>
...
function BalanceAdjustment({ wallet, language, onClose, onSaved }) {
  ...
  return <div className="modal-layer wallet-modal-layer"><button className="modal-backdrop" onClick={onClose} aria-label="Fechar" /><form className="modal-card wallet-modal wallet-adjustment-modal" onSubmit={submit}>
    ...
    <div className="wallet-modal-body form-stack">
      ...
      <div className="field-label"><span>Saldo real</span><input inputMode="decimal" value={form.actual_balance} onChange={(event) => setForm({ ...form, actual_balance: formatTypedMoneyForEditing(event.target.value, language) })} onBlur={() => setForm({ ...form, actual_balance: formatTypedMoneyAsCurrency(form.actual_balance, language) })} required autoFocus /></div>
      ...
      <div className="field-label"><span>Data</span><DateField value={form.date} onChange={(value) => setForm({ ...form, date: value })} /></div>
      <div className="field-label"><span>Observação <small>(opcional)</small></span><input value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Ex: conciliação do extrato" /></div>
    </div>
...
function TransferEditor({ wallets, initialSource, language, onClose, onSaved }) {
  ...
  return <div className="modal-layer wallet-modal-layer"><button className="modal-backdrop" onClick={onClose} aria-label="Fechar" /><form className="modal-card wallet-modal wallet-transfer-modal" onSubmit={submit}>
    ...
    <div className="wallet-modal-body form-stack">
      ...
      <div className="field-label"><span>Observação <small>(opcional)</small></span><input value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></div>
...
function ConsolidationEditor({ wallets, initialSource, language, onClose, onSaved }) {
  ...
  return <div className="modal-layer wallet-modal-layer"><button className="modal-backdrop" onClick={onClose} aria-label="Fechar" /><form className="modal-card wallet-modal wallet-transfer-modal wallet-consolidation-modal" onSubmit={submit}>
    ...
    <div className="wallet-modal-body form-stack">
      <div className="wallet-transfer-route">
        <div className="field-label"><span>Carteira que será esvaziada</span><WalletSelect wallets={active.filter((wallet) => String(wallet.id) !== form.destination_wallet_id)} value={form.source_wallet_id} onChange={(sourceId) => setForm((current) => ({ ...current, source_wallet_id: sourceId, destination_wallet_id: current.destination_wallet_id === sourceId ? String(active.find((wallet) => String(wallet.id) !== sourceId)?.id || "") : current.destination_wallet_id }))} ariaLabel="Carteiras de origem" /></div>
        <span className="wallet-transfer-direction" aria-hidden="true"><MoveRight size={17} /></span>
        <div className="field-label"><span>Carteira que receberá tudo</span><WalletSelect wallets={active.filter((wallet) => String(wallet.id) !== form.source_wallet_id)} value={form.destination_wallet_id} onChange={(destinationId) => setForm((current) => ({ ...current, destination_wallet_id: destinationId }))} ariaLabel="Carteiras de destino" /></div>
      </div>
